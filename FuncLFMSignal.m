...
switch LfmType
    case 1%正调频
        LfmSignal = Amplitude*exp(j*2*pi*CarrierFrequency*PulseTime+1j*pi*miu*PulseTime.^2);
    case 2%负调频
        LfmSignal = Amplitude*exp(j*2*pi*CarrierFrequency*PulseTime-1j*pi*miu*PulseTime.^2);
end
switch PriType
    case 1%重频固定
        ...
        SignalTemporary=[];
        for i=1:PriNum
            SignalTemporary=[SignalTemporary LfmSignal ZeroArray];
        end
    ...
    case 2%重频参差 脉冲间
        ...
        JHTemporary={};
        for i=1:100000
            if(mod(i*AntennaScanningSamplingCycle,roundn(CompleteSectionPri,-6))==0)
                JHTemporary{JTemporary}=i;
                JTemporary=JTemporary+1;
            end
        end
        AntennaScanningCycleNum=JHTemporary{1};
        PriNum=PulseUnevenPulseNum*(AntennaScanningCycleNum*AntennaScanningSamplingCycle)/roundn(CompleteSectionPri,-6);
        PriNum=round(PriNum);
        ...
        for JTemporary=1:PulseUnevenPulseNum
            ...
            ZeroArray=0*PriEcexptPulse;
            SignalTemporary{JTemporary}=[LfmSignal ZeroArray];
        end
        m=1;
        while(m<=PriNum)
            for JTemporary=1:PulseUnevenPulseNum
                if(m<=PriNum)
                    LfmSignalCell{m}=SignalTemporary{JTemporary};
                    m=m+1;
                end
            end
        end
        TotalLfmSignal=[];
        for i=1:PriNum
            TotalLfmSignal=[TotalLfmSignal LfmSignalCell{i}];
        end
        SignalExecptFirstToa=TotalLfmSignal;
    case 3%重频参差 脉组间
        %%%%%%%%%%%%%%%%%%%%%%%% 一段完整的组间参差信号周期%%%%%%%%%%%%%%%%%%%%%%%%%%%
        CompleteSectionPri=0;
        ...
        AntennaScanningCycleNum=JHTemporary{1};
        PriNum=PulseGroupUnevenGroupNum*PulseGroupUnevenPulseNumInGroup*(AntennaScanningCycleNum*AntennaScanningSamplingCycle)/roundn(CompleteSectionPri,-6);
        PriNum=round(PriNum);
        %%%%%%%%%%%%%%%%%%%% 生成一段完整组间参差Pri%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        m=1;
        ...
        while(m<=PriNum)
            for JTemporary=1:PulseGroupUnevenGroupNum % PulseGroupUnevenGroupNum 为组数
                ...
                for i=1:PulseGroupUnevenPulseNumInGroup
                    if(m<=PriNum)
                        TemporarySignal=[LfmSignal ZeroArray];
                        LfmSignalCell{m}=TemporarySignal;
                        m=m+1;
                    end
                end
            end
        ...
        while(m<=PriNum)
            for I=1:JTemporary
                if(m<=PriNum)
                    ...
                    ZeroArray=0*ZeroArraySamplingTime;
                    SignalTemporary=[LfmSignal ZeroArray];
                    LfmSignalCell{m}=SignalTemporary;
                    m=m+1;
                end
            end
        end
    ...
    case 7%重频滑变 正弦形状
        ...
        AntennaScanningCycleNum=JHTemporary{1};
        PriNum=SineSlidingNumOneCycle*(AntennaScanningCycleNum*length(AntennaScanningSamplingCycle))/length(CompletePriSamplingTime);
        PriNum=round(PriNum);

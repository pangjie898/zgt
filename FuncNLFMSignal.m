function [SignalExecptFirstToa,AntennaScanningCycleNum,RoundFirstPulseToa]=FuncNLFMSignal(BandWidth,PulseWidth,SamplingFrequency,Amplitude,CarrierFrequency,PriType,Pri,FirstPulseToa,PulseGroupUnevenGroupNum,PulseGroupUnevenPulseNumInGroup,OneTimeSlidingMaxRange,SawtoothSlidingNumOneCycle,TriangleSlidingNumOneCycle,SineSlidingNumOneCycle,PulseUnevenPulseNum,PulseUnevenPri,PulseGroupUnevenPri,JitterPercentage,AntennaScanningCycle)
%% 函数功能：产生非线性调频信号
%    输入：
%                        BandWidth：信号带宽；
%                       PulseWidth：脉冲宽度；
...
NlfmSignal=Amplitude*exp(j*2*pi*(CarrierFrequency*PulseTime-(BandWidth*PulseWidth/pi/4)*cos(2*pi*PulseTime/PulseWidth)));
switch PriType
    case 1%重频固定
        ...
        SignalTemporary=[];
        for i=1:PriNum
            SignalTemporary=[SignalTemporary NlfmSignal ZeroArray];
        end
    ...
    case 2%重频参差 脉冲间
        ...
        for JTemporary=1:PulseUnevenPulseNum
            ...
            ZeroArray=0*PriEcexptPulse;
            SignalTemporary{JTemporary}=[NlfmSignal ZeroArray];
        end
        m=1;
        while(m<=PriNum)
            for JTemporary=1:PulseUnevenPulseNum
                if(m<=PriNum)
                    NlfmSignalCell{m}=SignalTemporary{JTemporary};
                    m=m+1;
                end
            end
        end
        TotalNlfmSignal=[];
        for i=1:PriNum
            TotalNlfmSignal=[TotalNlfmSignal NlfmSignalCell{i}];
        end
        SignalExecptFirstToa=TotalNlfmSignal;
    case 3%重频参差 脉组间
        %%%%%%%%%%%%%%%%%%%%%%%% 一段完整的组间参差信号周期%%%%%%%%%%%%%%%%%%%%%%%%%%%
        CompleteSectionPri=0;
        ...
        while(m<=PriNum)
            for JTemporary=1:PulseGroupUnevenGroupNum % PulseGroupUnevenGroupNum 为组数
                ...
                for i=1:PulseGroupUnevenPulseNumInGroup
                    if(m<=PriNum)
                        TemporarySignal=[NlfmSignal ZeroArray];
                        NlfmSignalCell{m}=TemporarySignal;
                        m=m+1;
                    end
                end
            end
        ...
        while(m<=PriNum)
            for I=1:JTemporary
                if(m<=PriNum)
                    ...
                    ZeroArray=0*ZeroArraySamplingTime;
                    SignalTemporary=[NlfmSignal ZeroArray];
                    NlfmSignalCell{m}=SignalTemporary;
                    m=m+1;
                end
            end
        end

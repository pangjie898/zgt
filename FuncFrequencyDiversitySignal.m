...
%    输出：
%            SignalExecptFirstToa：频率分集（同时）信号；
%%
...
switch PriType
    case 1 %重频固定
        ...
        for i=1:FrequencyDiversityNum
            ...
            JitterPri{i}=10*exp(2*j*pi*Frequency*SignalSamplingTime);
        end
        FrequencyDiversitySignal=0;
        for i=1:FrequencyDiversityNum
            FrequencyDiversitySignal=FrequencyDiversitySignal+JitterPri{i};
        end
        TotalSig=FrequencyDiversitySignal/FrequencyDiversityNum;
        SignalExecptFirstToa=SignalArray.*TotalSig;
    case 2 %重频参差 脉冲间
        %%%%%%%%%%%%%%%%%%%% 生成一段完整脉冲间参差Pri%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        PulseTime= 0:1/SamplingFrequency:(PulseWidth-1/SamplingFrequency);%脉冲宽度，赋值1
        ...
        for i=1:FrequencyDiversityNum
            Frequency = FrequencyDiversityFrequencyNum(i);
            JitterPri{i}=10*i*exp(2*j*pi*Frequency*SignalSamplingTime);
        end
        FrequencyDiversitySignal=0;
        for i=1:FrequencyDiversityNum
            FrequencyDiversitySignal=FrequencyDiversitySignal+JitterPri{i};
    ...
    case 3 %重频参差 脉组间
        ...
        SignalArray=repmat(CompleteSignalArray,1,PriNum/(PulseGroupUnevenGroupNum*PulseGroupUnevenPulseNumInGroup));
        %%%%%%%%%%%%%%%%%%%% 生成信号%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        TotalSig=FrequencyDiversitySignal/FrequencyDiversityNum;
        JitterPri=TotalSig;
        SignalExecptFirstToa=SignalArray.*JitterPri;
    case 4 %重频抖动
        CenterPri=Pri;%频率抖动中心值
        PulseTime=0:1/SamplingFrequency:(PulseWidth-1/SamplingFrequency);
        SawtoothSlidingNumOneCycle=JitterPercentage;%频率抖动百分比
        %%%%%%%%%%%%%%%%%生成若干个抖动Pri%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for i=1:100000
            temp=-1+(1-(-1))*rand(1,1);%产生一个-1到1的随机数
            JitterPri(i)=CenterPri*(1+SawtoothSlidingNumOneCycle/100*temp);
            RoundJitterPri(i)=roundn(JitterPri(i),-6);
        end
        %%%%%%%%%%%%%%%%%%%根据天线扫描周期算出一段滑动信号的数目%%%%%%%%%%%%%%%%%%%
        n=ceil(2*SawtoothSlidingNumOneCycle/100*CenterPri/1e-6)+1;
        JTemporary=1;
        JHTemporary={};
        ...
        SignalArray=repmat(SignalArray,1,PriNum/JTemporary);
        %%%%%%%%%%%%%%%%%%%%%%%生成信号%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        SignalTime=PriNum/JTemporary*TotalPri;
        SignalSamplingTime= 0:1/SamplingFrequency:(SignalTime-1/SamplingFrequency);
        for i=1:FrequencyDiversityNum
            Frequency = FrequencyDiversityFrequencyNum(i);
            X{i}=10*i*exp(2*j*pi*Frequency*SignalSamplingTime);
        end
        FrequencyDiversitySignal=0;
        for i=1:FrequencyDiversityNum
            FrequencyDiversitySignal=FrequencyDiversitySignal+X{i};
        end
        TotalSig=FrequencyDiversitySignal/FrequencyDiversityNum;
        SignalExecptFirstToa=SignalArray.*TotalSig;
    case 5 % 重频滑变 锯齿形状
        ...
        for i=1:length(SlidingPriArray)
            SignalArray=[SignalArray SlidingPriArray{i}];
        end
    ...
    case 6 % 重频滑变 三角形状
        ...
        JHTemporary={};
        for i=1:100000
            if(mod(i*AntennaScanningCycle,(roundn(CompletePriTime,-7)))==0)
                JHTemporary{JTemporary}=i;
                JTemporary=JTemporary+1;
            end
        end
        AntennaScanningCycleNum=JHTemporary{1};
        PriNum=TriangleSlidingNumOneCycle*(AntennaScanningCycleNum*AntennaScanningCycle)/(roundn(CompletePriTime,-7));
        PriNum=round(PriNum);
        %%%%%%%%%%%%%%%%%%%% 生成完整信号的Pri%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        m=1;
